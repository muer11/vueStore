  ...
  export default {
    ...
    methods: {
      ...
      showProductList: function(){
        ...
        vm.axios({
          url: 'http://127.0.0.1:8080/listProduct',
          type: "GET",
          params: {
      ...
      changeInitQuantity: function(type, initQuantity, cartitemId){
        ...
        if(type == 0){ // type: 0-减少商品 1-增加商品 2-输入数量
          if(quantity == 1){
            alert("已是最小数量，再减就木有啦~")
            return false;
          }else{
      ...
      deleteCartitem: function(){
        ...
        if(vm.deleteitems.length > 0){
          vm.axios({
            url: 'http://127.0.0.1:8080/deleteItems',
            type: 'get',
            params:{
      ...
      goCheckout: function(){
        ...
        if(!vm.isUnabled){
          ...
          vm.$router.push({path:"/checkout", query:{cartItem: vm.cartItems}});
        }
      },
    }
  }
